cordova.define("cc.fovea.cordova.purchase.InAppPurchase", function (require, exports, module) { //
    // Cordova Purchase Plugin platform_www
    //
...
/*///*/     store.PURCHASE_EXPIRED = 6778003;
    ...
    (function () {
        ...
        ///
        store.register = function (product, offline = false) {
            if (!product)
                return;
            if (!product.length)
                store.register([product], offline);
            else
                registerProducts(product, offline);
        };
        ...
        //
        // See documentation for the [product](#product) object for more information.
        function registerProducts(products, offline) {
            for (var i = 0; i < products.length; ++i) {
                ...
                if (hasKeyword(p.id) || hasKeyword(p.alias))
                    continue;
                if (offline || store.sandbox) {
                    if (products[i].productId) {
                        var ap = {};
                        ap.productId = products[i].productId;
                        if (products[i].title) ap.title = products[i].title;
                        if (products[i].description) ap.description = products[i].description;
                        if (products[i].price) ap.price = products[i].price;
                        if (products[i].price_amount_micros) ap.price_amount_micros = products[i].price_amount_micros;
                        if (products[i].price_currency_code) ap.price_currency_code = products[i].price_currency_code;
                        store.products.availableProducts.push(ap);
                    }
                    if (products[i].owned) {
                        var pp = {};
                        pp.productId = products[i].productId;
                        if (products[i].license) pp.license = products[i].license;
                        store.products.purchasedProducts.push(pp);
                    }
                    var ad = {};
                    if (products[i].orderId) ad.orderId = products[i].orderId;
                    if (products[i].purchaseState) ad.purchaseState = products[i].purchaseState;
                    if (products[i].transaction) ad.transaction = products[i].transaction;
                    store.products.additionalData[products[i].productId] = ad;
                }
    ...
    (function () {
        ...
        store.products.availableProducts = [];
        store.products.purchasedProducts = [];
        store.products.additionalData = [];
    ...
    (function () {
        ...
        InAppBilling.prototype.getPurchases = function (success, fail) {
            ...
            success(store.products.purchasedProducts);
        ...
        InAppBilling.prototype.buy = function (success, fail, productId, additionalData) {
            ...
            success(store.products.additionalData[productId]);//productId);
        ...
        InAppBilling.prototype.getAvailableProducts = function (success, fail) {
            ...
            success(store.products.availableProducts);
    ...
    (function () {
        ...
        store.setProductData = function (product, data) {
            ...
            if (transaction) {
                product.transaction = {
                    type: 'browser-store-transaction',
